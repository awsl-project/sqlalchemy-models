...
class ChatgptConversation(Base):
    __tablename__ = 'chatgpt_conversation'

    id = Column(INT, primary_key=True, autoincrement=True)
    chat_id = Column(String(255), index=True, unique=True)
...
class ApiConfig(Base):
    ...
    def set(key: str, value: str, session) -> bool:
        if not key:
            return False
        config = session.query(
            ApiConfig
        ...
        if config:
            ...
            session.add(ApiConfig(key=key, value=value))
        session.commit()
